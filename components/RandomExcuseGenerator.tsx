...
export function RandomExcuseGenerator() {
  ...
  const fetchUsage = async () => {
    try {
      const response = await fetch('/api/random-excuse/usage', {
        credentials: 'include',
      });
      if (response.ok) {
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>🎭 Random Excuse Generator</CardTitle>
          <CardDescription>
            Generate creative, believable (or hilariously unbelievable) excuses for any situation.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="situation"
              placeholder="e.g., 'being late to work', 'missing a deadline', 'not attending an event'"
              value={situation}
          ...
          <div className="space-y-2">
            <Label htmlFor="believability">Believability</Label>
            <select
              id="believability"
              value={believability}
              onChange={(e) => setBelievability(e.target.value)}
              className="w-full px-3 py-2 border rounded-md bg-background"
            >
              <option value="believable">Believable</option>
              <option value="unbelievable">Unbelievable (Funny)</option>
              <option value="hilarious">Hilariously Over-the-Top</option>
            </select>
          </div>

          <div className="space-y-2">
            <Label htmlFor="count">Number of Excuses</Label>
            <Input
              id="count"
              type="number"
              min="1"
      ...
      {excuses.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Generated Excuses</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {excuses.map((excuse, idx) => (
              ...
              >
                <p className="text-sm font-medium mb-1">Excuse {idx + 1}:</p>
                <p>{excuse}</p>
              </div>
            ))}
          </CardContent>
